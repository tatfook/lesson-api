...
module.exports = app => {
    ...
    model.classroomLog = async ({
        ...
        if (action === 'create') {
            ...
            const [ pkg, lesson, packageLesson ] = await Promise.all([
                ...
                app.model.PackageLesson.findOne({
    ...
    model.classLog = async ({
        ...
        const paramsBegin = params.begin
            ? moment(new Date(params.begin)).format('YYYY/MM/DD')
            : begin;
        const paramsEnd = params.end
            ? moment(new Date(params.end)).format('YYYY/MM/DD')
            : end;
        ...
        if (action === 'createClass') {
            return await app.model.LessonOrganizationLog.create({
                organizationId,
                type: '班级',
                description: `新建, 班级: ${cls.name}, 开班时间 ${begin} - ${end}`,
                username,
                handleId,
            });
        }
        if (action === 'updateClass') {
            if (cls.name !== params.name) {
                await app.model.LessonOrganizationLog.create({
                    organizationId,
                    type: '班级',
                    description: `改名, 班级: ${cls.name}, 修改为: ${params.name}`,
                    username,
                    handleId,
                });
            }
            if (paramsBegin !== begin || paramsEnd !== end) {
                await app.model.LessonOrganizationLog.create({
                    organizationId,
                    type: '班级',
                    description: `修改有效期, 班级: ${cls.name}, 开班时间修改为: ${paramsBegin} - ${paramsEnd}`,
                    username,
                    handleId,
                });
            }
            ...
            await app.model.LessonOrganizationLog.create({
                organizationId,
                type: '班级',
                description: `生成邀请码: ${cls.name}, 生成 ${count} 个`,
                username,
                handleId,
            });
        }
    ...
    model.studentLog = async ({
        ...
        if (
            ...
            if (roleId === CLASS_MEMBER_ROLE_TEACHER) {
                const member = _.find(
                    oldmembers,
                    o =>
                        o.classId === 0 && o.roleId && CLASS_MEMBER_ROLE_TEACHER
                );
                if (member) {
                    return await app.model.LessonOrganizationLog.create({
                        organizationId,
                        type: '老师',
                        description: `改名, 教师: ${member.realname ||
                            ''} 修改为: ${realname}`,
                        username,
                        handleId,
                    });
            ...
            if (index < 0 && cls) {
                if (roleId === CLASS_MEMBER_ROLE_STUDENT) {
                    await app.model.LessonOrganizationLog.create({
                        organizationId,
                        type: '班级',
                        description: `移除学生, ${cls.name}, 移除学生: ${member.realname}`,
                        handleId,
                        username,
                    });
                }
                if (roleId === CLASS_MEMBER_ROLE_TEACHER) {
                    await app.model.LessonOrganizationLog.create({
                        organizationId,
                        type: '班级',
                        description: `移除老师, ${cls.name}, 移除老师: ${member.realname}`,
                        handleId,
                        username,
                    });
                }
                ...
                if (member.realname !== realname && realname) {
                    if (roleId === CLASS_MEMBER_ROLE_STUDENT) {
                        await app.model.LessonOrganizationLog.create({
                            organizationId,
                            type: '学生',
                            description: `改名, 学生: ${member.realname ||
                                ''}, 修改为: ${realname}`,
                            handleId,
                            username,
                        });
        ...
        for (let i = 0; i < classIds.length; i++) {
            ...
            const member = _.find(
                oldmembers,
                o => o.classIds === classId && o.roleId & roleId
            );
            if (member) continue;
            ...
            if (roleId === CLASS_MEMBER_ROLE_STUDENT) {
                await app.model.LessonOrganizationLog.create({
                    organizationId,
                    type: '班级',
                    description: `添加学生, ${cls.name}, 添加学生: ${realname}`,
                    handleId,
                    username,
                });
            }
            if (roleId === CLASS_MEMBER_ROLE_TEACHER) {
                await app.model.LessonOrganizationLog.create({
                    organizationId,
                    type: '班级',
                    description: `添加老师, ${cls.name}, 添加老师: ${realname}`,
                    handleId,
                    username,
                });
            }
