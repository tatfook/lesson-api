...
const { TRADE_TYPE_LESSON_STUDY } = require('../common/consts.js');
...
class LearnRewardService extends Service {
    ...
    async getRewards(userId, packageId, lessonId) {
        ...
        // 是否已领取
        let lessonReward = await this.ctx.model.LessonReward.findOne({
            where,
        }).then(o => o && o.toJSON());
        ...
        const beanCount = lessonReward.bean ? 0 : TEN; // 已奖励则不再奖励
        const coinCount =
            account.lockCoin < TEN || lessonReward.coin
                ? 0
                : _.random(
                    TEN,
                    account.lockCoin > FIFTY ? FIFTY : account.lockCoin
                );

        lessonReward.coin = lessonReward.coin + coinCount;
        lessonReward.bean = lessonReward.bean + beanCount;
        if (~~coinCount === 0 && ~~beanCount === 0) {
        ...
        const [ lesson ] = await Promise.all([
            ...
            // 扣除用户可返还余额
            this.ctx.service.keepwork.accountIncrement(
                { coin: coinCount, bean: beanCount, lockCoin: 0 - coinCount },
                userId
            ),
        ]);
    ...
    async getByCondition(condition) {
        let data = await this.ctx.model.LessonReward.findOne({
            where: condition,
        });
