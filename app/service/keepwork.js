...
class KeepworkService extends Service {
	...
	async getAllUserByCondition(condition) {
		const list = await this.ctx.helper.curl("get", `${this.baseUrl}users`, condition, {}, true);
		return list ? list : this.ctx.throw(500, Err.UNKNOWN_ERR);
	}


	async getUserById(userId) {
		const ret = await this.ctx.helper.curl("get", `${this.baseUrl}users/${userId}`);
	...
	async getUserDatas(userId) {
		const ret = await this.ctx.helper.curl("get", `${this.baseUrl}lessons/userdatas`, { id: userId, apiKey });
		return ret;
	}

	// 更新token
	async setUserDatas(userId, data) {
		const ret = await this.ctx.helper.curl("post", `${this.baseUrl}lessons/userdatas`, Object.assign({ id: userId, apiKey }, data));
		return ret;
	}
	...
	async getAccountsAndRoles(userId) {
		const ret = await this.ctx.helper.curl("get", `${this.baseUrl}lessons/accountsAndRoles`, { userId, apiKey });
		return ret;
	}

	// 增加数值，调用increment
	async accountIncrement(incrementObj, userId) {
		const ret = await this.ctx.helper.curl("put", `${this.baseUrl}lessons/accountsIncrement`, { incrementObj, userId, apiKey });
		return ret;
	}

	// 查找accounts，调用findOne
	async getAccounts(userId) {
		const ret = await this.ctx.helper.curl("get", `${this.baseUrl}lessons/accounts`, { userId, apiKey });
		return ret;
	}
