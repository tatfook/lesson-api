...
class EvalReportService extends Service {
	...
	// 获取对班级classId发起的点评
	async getReportList({ classId, userId, roleId, name = undefined, type = undefined, days }) {
		const list = await this.ctx.model.EvaluationReport.getReportList({ classId, roleId, userId, name, type, days });
		return list;
	}
