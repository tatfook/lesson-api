...
class LessonOrgIndexService extends Service {
	...
	async changePassword(params, authParams) {
		...
		if (roleId < CLASS_MEMBER_ROLE_ADMIN) {
			...
			if (teacher.roleId < CLASS_MEMBER_ROLE_TEACHER) return this.throw(400, Err.AUTH_ERR);
		}

		const member = await this.ctx.service.lessonOrganizationClassMember.getByCondition({
			organizationId, classId: params.classId, memberId: params.memberId
		});
	...
	async getLog(queryOptions, condition) {
		const logs = await this.ctx.model.LessonOrganizationLog.findAndCountAll({ ...queryOptions, where: condition });
		return logs;
	}
}
