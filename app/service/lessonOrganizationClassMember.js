...
class LessonOrgClassMemberService extends Service {
    ...
    async toFormal(
        ...
        try {
            ...
            const endTime = _.min([endTimeMap[type](), moment(org.endDate).format('YYYY-MM-DD')]);
            // 创建成员
            const objs = [];
            for (let i = 0; i < userIds.length; i++) {
    ...
    async recharge(
        ...
        try {
            ...
            for (let i = 0; i < userIds.length; i++) {
                ...
                const endTime = _.min([endTimeMap[type](oldEndTime), moment(org.endDate).format('YYYY-MM-DD')]);
