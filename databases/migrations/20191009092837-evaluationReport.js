...
module.exports = {
	up: (queryInterface, Sequelize) => {
		...
		return queryInterface.sequelize.transaction(t => Promise.all([
			queryInterface.addColumn("lessonOrganizations", "QRCode", { type: Sequelize.STRING(256), comment: "二维码链接url" }, { transaction: t }),
			queryInterface.addColumn("lessonOrganizations", "propaganda", { type: Sequelize.STRING(256), comment: "宣传文案" }, { transaction: t }),
			...
			queryInterface.createTable("evaluationUserReports", {
				...
				await queryInterface.addIndex("evaluationUserReports", ["userId"]);
				await queryInterface.addIndex("evaluationUserReports", ["reportId"]);
				await queryInterface.addIndex("evaluationUserReports", ["userId", "reportId"], { unique: true });
			}),
			...
			queryInterface.createTable("evaluationReports", {
				...
				await queryInterface.addIndex("evaluationReports", ["userId"]);
				await queryInterface.addIndex("evaluationReports", ["classId"]);
			})
		]));
	},
