...
describe("test/model/evaluationReport.test.js", () => {
	before(async () => {
		...
		await app.model.EvaluationReport.create({ name: "什么报告", type: 1, classId: 1, userId: 1 });
		await app.model.EvaluationReport.create({ name: "什么报告2", type: 2, classId: 2, userId: 1 });
	});

	it("001 getUserReportList 未点评名单", async () => {
		const list = await app.model.EvaluationUserReport.getUserReportList({ reportId: 1, status: 1 });
		assert(list.length === 2 && list[0].studentId === 2 && list[1].studentId === 4);
	});

	it("002 getUserReportList 已点评名单", async () => {
		const list = await app.model.EvaluationUserReport.getUserReportList({ reportId: 1, status: 2 });
	...
	it("005 getTeacherByUserReportId 获取老师id", async () => {
		const ret = await app.model.EvaluationUserReport.getTeacherByUserReportId(1);
		assert(ret.teacherId === 1 && ret.studentId === 2);
	});

	it("006 getStudentIdsByReportId 获取这个报告中已经点评了的学生id", async () => {
		const ret = await app.model.EvaluationUserReport.getStudentIdsByReportId(1);
		assert(ret.length === 1 && ret[0].studentId === 2);
	});

	it("007 getByUserIdAndClassIds", async () => {
		const ret = await app.model.EvaluationUserReport.getByUserIdAndClassIds(2, [1]);
		assert(ret.length === 1 && ret[0].classId === 1);
	});

	it("008 getReportAndOrgNameById ", async () => {
		const ret = await app.model.EvaluationUserReport.getReportAndOrgNameById(1);
		assert(ret.orgName === "什么机构"
			&& ret.realname === "什么学生"
			&& ret.teacherName === "什么老师");
	});

	it("009 getClassmatesAvgStarById", async () => {
		const ret = await app.model.EvaluationUserReport.getClassmatesAvgStarById(1);
		assert(ret.starAvg === "2.00"
			&& ret.spatialAvg === "3.00"
	...
	it("010 getClassmatesHistoryAvgStar", async () => {
		const ret = await app.model.EvaluationUserReport.getClassmatesHistoryAvgStar(1);
		assert(ret.starAvg === "2.00"
			&& ret.spatialAvg === "3.00"
			&& ret.collaborativeAvg === "4.00"
			&& ret.creativeAvg === "1.00"
	...
	it("012 getUserHistoryStar", async () => {
		const ret = await app.model.EvaluationUserReport.getUserHistoryStar(2, 1);
		assert(ret.length === 1);
		assert(ret[0].star === 2
			&& ret[0].spatial === 3
	...
	it("013 getClassmatesHistoryAvgStarGroupByReportId", async () => {
		const ret = await app.model.EvaluationUserReport.getClassmatesHistoryAvgStarGroupByReportId(1);
		assert(ret.length === 1);
		assert(ret[0].starAvg === "2.00"
	...
	it("014 getEvaluationCommentListSql", async () => {
		const ret = await app.model.EvaluationUserReport.getEvaluationCommentListSql(2, 1);
		assert(ret.length === 1);
		assert(ret[0].star === 2
	...
	it("015 getClassAndEvalStatus", async () => {
		const ret = await app.model.EvaluationUserReport.getClassAndEvalStatus(1);
		assert(ret.length === 2);
	...
	it("016 getTeacherCommentStatistics", async () => {
		const ret = await app.model.EvaluationUserReport.getTeacherCommentStatistics(1);
		assert(ret.length === 1
