...
describe('test/service/lessonOrganizationPackage.test.js', async () => {
    ...
    describe('updateLessonNo',async()=>{
        ...
        it('001',async()=>{
            const ctx = app.mockContext();
            await ctx.service.lessonOrganizationPackage.updateLessonNo(1,[{lessonId:1,lessonNo:9}]);
            const ret = await ctx.model.LessonOrganizationPackage.findOne({where:{ id:pkgs.id }});
