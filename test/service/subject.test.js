...
describe('test/service/subject.test.js',async()=>{
	describe('findAllByCondition',async()=>{
		...
		it('001',async()=>{
			...
			const ret = await ctx.service.subject.findAllByCondition({subjectName:'偷'});
			assert(ret.length===1);
		});
	});
	...
	describe('getByCondition',async()=>{
		...
		it('001',async()=>{
            const ctx = app.mockContext();
			const ret = await ctx.service.subject.getByCondition({subjectName:'偷'});
			assert(ret);
		});
	});
	...
	describe('createSubject',async()=>{
		it('001',async()=>{
            const ctx = app.mockContext();
			const ret = await ctx.service.subject.createSubject({subjectName:'偷'});
			assert(ret);
		});
	});
	...
	describe('updateByCondition',async()=>{
		...
		it('001',async()=>{
            const ctx = app.mockContext();
			await ctx.service.subject.updateByCondition({subjectName:'摸'},{id:1});
