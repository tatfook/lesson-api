...
describe("test/service/evaluationReport.test.js", () => {
	...
	it("001 createEvalReport 创建点评记录 应该成功", async () => {
		...
		const ret = await ctx.service.evaluationReport.createEvalReport({ userId: 1, name: "报告名字", type: 1, classId: 1 });
		assert(ret);

		// 检查是否真的创建
		const report = await app.model.EvaluationReport.findOne({ where: { name: "报告名字" }});
		assert(report.id === 1);
	});

	it("002 updateEvalReport 更新发起的点评记录 应该成功", async () => {
		const ctx = app.mockContext();
		const ret = await ctx.service.evaluationReport.updateEvalReport({ name: "这个名字修改了", type: 2 }, { name: "报告名字" });
		assert(ret);

		const report = await app.model.EvaluationReport.findOne({ where: { id: 1 }});
	...
	it("003 createUserReport 创建对学生点评的记录 应该成功", async () => {
		...
		// 检查是否真的创建
		const userReport = await app.model.EvaluationUserReport.findOne({ where: { userId: 2 }});
	...
	it("004 createUserReport 同一个点评重复创建对学生点评的记录 应该失败", async () => {
		...
		// 检查是否真的创建
		const userReport = await app.model.EvaluationUserReport.findAll({ where: { userId: 2 }});
		assert(userReport.length === 1);
	});

	it("005 updateUserReportByCondition 修改对学生的点评 应该成功", async () => {
		const ctx = app.mockContext();
		const ret = await ctx.service.evaluationReport.updateUserReportByCondition({ comment: "文字评价修改了" }, { id: 1 });
		assert(ret);

		// 检查是否真的修改
		const userReport = await app.model.EvaluationUserReport.findOne({ where: { id: 1 }});
	...
	it("006 getStudentIdsByReportId 获取这个报告中已经点评了的学生id", async () => {
		...
		const ret = await ctx.service.evaluationReport.getStudentIdsByReportId(1);
	...
	it("008 destroyReportById 删除发起的点评，那么已经点评过的记录也要删除", async () => {
		...
		// 后置操作 恢复数据
		const ret4 = await ctx.service.evaluationReport.createEvalReport({ userId: 1, name: "报告名字", type: 1, classId: 1 });
	...
	it("009 getUserReportByCondition 获取一条对学生的点评", async () => {
		...
		const ret = await ctx.service.evaluationReport.getUserReportByCondition({ id: 3 });
		assert(ret);
	});
	...
	it("010 getUserReportAndOrgInfo ", async () => {
		...
		const ret = await ctx.service.evaluationReport.getUserReportAndOrgInfo(3);
		assert(ret);
	});
	...
	it("011 getReportByCondition 获取一条发起的点评", async () => {
		...
		const ret = await ctx.service.evaluationReport.getReportByCondition({ name: "报告名字" });
		assert(ret);
	});
	...
	it("012 getUserReportList 获取对学生的点评列表", async () => {
		...
		const ret = await ctx.service.evaluationReport.getUserReportList({ reportId: 2 });
	...
	it("013 getTeacherByUserReportId 根据userReportId获取老师的id", async () => {
		...
		const ret = await ctx.service.evaluationReport.getTeacherByUserReportId(3);
	...
	it("014 destroyUserReportByCondition 根据条件删除对学生的点评", async () => {
		...
		const ret = await ctx.service.evaluationReport.destroyUserReportByCondition({ id: 3 });
		assert(ret);
	...
	it("015 getClassmatesAvgStarById 本班同学本次点评的平均能力值", async () => {
		const ctx = app.mockContext();
		// 这时候只有他一个人被老师点评了
		const ret = await ctx.service.evaluationReport.getClassmatesAvgStarById({ reportId: 2 });
		assert(ret.starAvg === "3.00"
			&& ret.spatialAvg === "4.00"
	...
	it("016 getClassmatesAvgStarById 本班同学本次点评的平均能力值", async () => {
		...
		const ret = await ctx.service.evaluationReport.getClassmatesAvgStarById({ reportId: 2, refresh: true });
		assert(ret.starAvg === "4.00"
			&& ret.spatialAvg === "3.00"
			&& ret.collaborativeAvg === "3.00"
			&& ret.creativeAvg === "3.00"
			&& ret.logicalAvg === "5.00"
	...
	it("017 getClassmatesHistoryAvgStar 本班同学历次能力值总和的平均值", async () => {
		const ctx = app.mockContext();
		// 这时候只有一次点评
		const ret = await ctx.service.evaluationReport.getClassmatesHistoryAvgStar({ classId: 1, refresh: true });
		assert(ret.starAvg === "4.00"
			&& ret.spatialAvg === "3.00"
			&& ret.collaborativeAvg === "3.00"
			&& ret.creativeAvg === "3.00"
	...
	it("018 getClassmatesHistoryAvgStar 本班同学历次能力值总和的平均值", async () => {
		const ctx = app.mockContext();
		// 创建第二次点评
		await ctx.service.evaluationReport.createEvalReport({ userId: 1, name: "报告名字2", type: 1, classId: 1 });
		await ctx.service.evaluationReport.createUserReport({
			userId: 2,
			reportId: 3,
			star: 3,
	...
	it("019 getUserSumStar 获取学生在这个班历次能力值总和", async () => {
		...
		const ret = await ctx.service.evaluationReport.getUserSumStar({ classId: 1, studentId: 2, refresh: true });
		assert(ret.starCount === "6"
			&& ret.spatialCount === "8"
	...
	it("020 getUserHistoryStar 获取学生在这个班历次成长", async () => {
		...
		const ret = await ctx.service.evaluationReport.getUserHistoryStar({ classId: 1, studentId: 2, refresh: true });
		assert(ret.length === 2);
		assert(ret[0].star === 3 && ret[0].spatial === 4
			&& ret[0].collaborative === 3
			&& ret[0].creative === 3
			&& ret[0].logical === 5
			&& ret[0].compute === 2
			&& ret[0].coordinate === 3);
		assert(ret[1].star === 3 && ret[1].spatial === 4
	...
	it("021 getClassmatesHistoryAvgStarGroupByReportId 获取同学历次成长的平均值", async () => {
		...
		const ret = await ctx.service.evaluationReport.getClassmatesHistoryAvgStarGroupByReportId({ classId: 1, refresh: true });
		assert(ret.length === 2);
		assert(ret[0].starAvg === "4.00"
	...
	it("022 getUserReportDetail 学生获得的点评详情 ", async () => {
		...
		const ret = await ctx.service.evaluationReport.getUserReportDetail(4, 2, 1, 1);
		...
		assert(classmatesAvgStar.starAvg === "4.00"
		...
		assert(historyStarStatistics.classmatesHistoryAvgStar.length === 0
	...
	it("023 updateUserReportByCondition 修改对学生的点评", async () => {
		...
		let ret = await ctx.service.evaluationReport.updateUserReportByCondition({ comment: "哈哈哈哈" }, { id: 4 });
		assert(ret);
		ret = await app.model.EvaluationUserReport.findOne({ where: { id: 4 }});
	...
	it("024 getEvaluationCommentList 学生获得的历次点评列表", async () => {
		...
		const ret = await ctx.service.evaluationReport.getEvaluationCommentList(1, 2);
