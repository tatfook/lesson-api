...
describe('test/service/lessonOrganizationClass.test.js', async () => {
	describe('getByCondition', async () => {
		...
		it('002', async () => {
			const ctx = app.mockContext();
			const ret = await ctx.service.lessonOrganizationClass.getByCondition({ name: cls.name });
			assert(ret.name === cls.name);
		});
	});
	...
	describe('updateByCondition', async () => {
		...
		it('001', async () => {
			const ctx = app.mockContext();
			const ret = await ctx.service.lessonOrganizationClass.updateByCondition({ name: 'updated' }, { id: cls.id });
			assert(ret[0] === cls.id);
		});
	});
	...
	describe('historyClass', async () => {
		let cls;
		beforeEach(async () => {
			cls = await app.factory.create('LessonOrganizationClass', { status: 2 });
			app.mockService('keepwork', 'getAllUserByCondition', () => []);
		});

		it('001', async () => {
			const ctx = app.mockContext();
			const ret = await ctx.service.lessonOrganizationClass.historyClass({ offset: 0, limit: 10 }, cls.organizationId);
	...
	describe('findAllByCondition', async () => {
		let cls;
		beforeEach(async () => {
			cls = await app.factory.create(
				'LessonOrganizationClass',
			);
		});
	...
	describe('updateClass 更新班级', async () => {
		...
		it('001', async () => {
			...
			await ctx.service.lessonOrganizationClass.updateClass({
				...
				name: '班级名称',
				id: cls.id
			}, { organizationId: cls.organizationId, roleId: 64, userId: 1, username: '' });
		});
	});
	...
	describe('destroyClass', async () => {
		...
		it('001', async () => {
			const ctx = app.mockContext();
			await ctx.service.lessonOrganizationClass.destroyClass(cls.id, cls.organizationId);
	...
	describe('classLastestProject 班级最近项目', async () => {
		...
		it('001', async () => {
			const ctx = app.mockContext();
			const ret = await ctx.service.lessonOrganizationClass.classLastestProject(member.classId, member.organizationId);
			assert(ret);
		});
	});
	...
	describe('closeClass 关闭班级', async () => {
		let cls;
		beforeEach(async () => {
			cls = await app.factory.create('LessonOrganizationClass', { status: 1 });
		});
		it('001', async () => {
			const ctx = app.mockContext();
			await ctx.service.lessonOrganizationClass.closeClass(cls.id);
			const ret = await app.model.LessonOrganizationClass.findOne({ where: { id: cls.id } });
			assert(ret.status === 2);
		});
	});
});
