...
describe('test/service/userMessage.test.js', () => {
	...
	beforeEach(async () => {
		...
		const msg = await app.model.Message.findOne({ where: { id: userMsg.msgId } });
		organizationId = msg.organizationId;

		const org = await app.model.LessonOrganization.findOne({ where: { id: organizationId } });
		orgName = org.name;
	});
	...
	describe('getIndexOfMessage', () => {
		it('001', async () => {
			...
			const ret = await ctx.service.userMessage.getIndexOfMessage(msgId, organizationId);
			assert(ret === 0);
		});
	});
});
